...
1. **Dispatch `loop-planner`** (Fable; fall back to `general-purpose` with `model: fable` if the type is unavailable) with: the goal or phase scope, the plan file path, a **read list**, the ledger path, and — when re-planning — the previous plan plus what remains of the goal. The read list is the KB entry point plus the pages, contract files and decision-archive sections the scope touches, picked from the KB index; unsure about a module → list its contract, not the whole KB. Never paste the KB into the prompt; the planner reads the list and names anything it had to read beyond it. Record the planner dispatch's token count in the ledger (planning cost is otherwise unmeasured).
